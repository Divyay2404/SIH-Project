...
Implements dynamic 2-Mark, 5-Mark, and 10-Mark prompt schemas.
...
class MarksAwareRAGEngine:
    ...
    def answer_question(self, question: str, marks: int = 5) -> Dict[str, Any]:
        ...
        and generates marks-scaled response with citation metadata.
        """
        ...
        # Step 3: Format Marks-Aware Output according to rubric matrix
    ...
    def _format_marks_aware_output(self, question: str, context: str, marks: int) -> str:
        """Applies 2-Mark, 5-Mark, or 10-Mark academic output prompt matrix."""
        q_lower = question.lower()

        if marks == 2:
            # 2-Mark Schema: Definition Scale (Concise 1-2 sentences + 1 example, strictly <50 words)
            if "delete" in q_lower or "deletion" in q_lower:
                return (
                    "**2-MARK ANSWER (Definition Scale)**\n\n"
                    "**Definition**: BST Deletion removes a target node while preserving node ordering rules.\n"
                    "**Example**: Deleting a leaf node unlinks its parent pointer to NULL."
                )
            elif "insert" in q_lower or "insertion" in q_lower:
                return (
                    "**2-MARK ANSWER (Definition Scale)**\n\n"
                    "**Definition**: BST Insertion places a key in its leaf position based on key comparisons.\n"
                    "**Example**: Inserting 15 into root 20 recurses to the left branch."
                )
            else:
                return (
                    "**2-MARK ANSWER (Definition Scale)**\n\n"
                    f"**Definition**: {context[:120]}.\n"
                    "**Example**: Search operation runs in O(log N) average time."
                )

        elif marks == 5:
            # 5-Mark Schema: Concept Scale (Paragraph definition, 3-4 bullet points, code/process example)
            if "delete" in q_lower or "deletion" in q_lower:
                return (
                    "**5-MARK ANSWER (Concept Scale)**\n\n"
                    "**Overview**: BST Deletion removes a node while ensuring all remaining nodes satisfy the BST invariant (`Left < Root < Right`).\n\n"
                    "**Core Principles & Cases**:\n"
                    "• **Case 1 (Leaf Node)**: Delete node directly by setting parent reference to NULL.\n"
                    "• **Case 2 (Single Child)**: Replace the node pointer directly with its existing child.\n"
                    "• **Case 3 (Two Children)**: Substitute node value with its **in-order successor** (minimum value in right subtree), then recursively delete successor.\n\n"
                    "**Process Flow Example**:\n"
                    "`Delete node 50 (children 30 & 70) -> Replace 50 with in-order successor 60 -> Delete original node 60.`"
                )
            else:
                return (
                    "**5-MARK ANSWER (Concept Scale)**\n\n"
            ...
            # 10-Mark Schema: Comprehensive Essay Scale (Abstract definition, advantages, detailed algorithm, step-by-step math proof/diagram text, evaluative conclusion)
            return (
                "**10-MARK ANSWER (Comprehensive Essay Scale)**\n\n"
                "### 1. Abstract & Academic Definition\n"
                "A **Binary Search Tree (BST)** is a non-linear hierarchical data structure. Every node satisfies the invariant: `Key(Left Subtree) < Key(Root) < Key(Right Subtree)`.\n\n"
                "### 2. Advantages\n"
                "• Dynamic allocation without contiguous memory overhead.\n"
                "• Efficient $\\mathcal{O}(\\log N)$ search, insertion, and deletion on balanced trees.\n"
                "• Natural sorted retrieval via In-Order traversal.\n\n"
                ...
                "Standard BSTs offer rapid average operations, but degenerate under sorted inputs. For guaranteed performance, self-balancing architectures (AVL or Red-Black Trees) are recommended."
            )
