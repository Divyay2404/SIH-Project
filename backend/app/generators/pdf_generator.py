...
class StudyHandoutGenerator:
    ...
    def generate_handout_pdf(self, document: Dict[str, Any]) -> bytes:
        ...
        story = [
            Paragraph(f"B.Tech Study Guide: {self._safe_text(title)}", title_style),
            Paragraph("Verified Academic Knowledge Base | Generated by StudyForge OS", subtitle_style),
        ...
        story.append(Paragraph(f"<b>2 Marks:</b> Define the fundamental concept of {self._safe_text(title)}.", body_style))
        story.append(Paragraph(f"<b>5 Marks:</b> Explain the core mechanism or architecture of {self._safe_text(title)}.", body_style))
        story.append(Paragraph(f"<b>10 Marks:</b> Provide an in-depth analytical evaluation and step-by-step procedure for {self._safe_text(title)}.", body_style))
